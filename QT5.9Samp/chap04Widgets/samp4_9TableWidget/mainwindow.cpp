...
//    labStudID->setMinimumWidth(200);wwwwwww
...
//void MainWindow::on_btnSetWidget_clicked()
//{ //为“民族”一列添加下拉列表框，
//    int i;
//    QComboBox   *cBoxNation;
//    QStringList strList;
//    strList<<"汉族"<<"回族"<<"壮族"<<"藏族"<<"苗族";

//    for (i=0;i<ui->tableInfo->rowCount();i++)
//    {
//        cBoxNation=new QComboBox(ui->tableInfo);
//        cBoxNation->addItems(strList);
//        ui->tableInfo->setCellWidget(i,colNoNation,cBoxNation); //为单元格设置一个控件，但是这个控件并未起到编辑作用
//    }

//}


//void MainWindow::on_tableInfo_cellDoubleClicked(int row, int column)
//{ //双击时进入编辑状态，加入一个QComboBox进行选择
//    QComboBox   *cBoxSex;

//    if (column==colNoSex)
//    {
//        cBoxSex=new QComboBox(ui->tableInfo);
//        cBoxSex->addItem("男");
//        cBoxSex->addItem("女");
//        ui->tableInfo->setCellWidget(row,column,cBoxSex);//为单元格添加一个空间
//    }
//}

//void MainWindow::on_tableInfo_cellChanged(int row, int column)
//{  //希望在单元格完成编辑后删除编辑器，但是不知如何实现


////    if (column==colNoSex)
////    {
////        if (ui->tableInfo->cellWidget(row,column) != NULL) //获取单元格的控件
////            ui->tableInfo->removeCellWidget(row,column); //删除控件
////    }
...
//void MainWindow::on_btnTextToTable_clicked()
//{
//    int i,cnt;
//    QString str;
...
//    doc=ui->textEdit->document(); //QPlainTextEdit 的内容保存在一个 QTextDocument 里
//    cnt=doc->blockCount();//QTextDocument分块保存内容，文本文件就是硬回车符是一个block,

//    ui->textEdit->appendPlainText(""); //增加一个空行
//    ui->textEdit->appendPlainText(""); //增加一个空行

//    for (i=0; i<cnt;i++) //扫描所有 blobk
//    {
//         textLine=doc->findBlockByNumber(i);//用blobk编号获取block，就是获取一行
//         str=textLine.text(); //转换为文本
//         ui->textEdit->appendPlainText(str); //文本添加到QPlainTextEdit的显示内容
//    }
//}
...
//    int cellType=current->type();//获取单元格的类型
//    labCellType->setText(QString::asprintf("当前单元格类型：%d",cellType));

//    QTableWidgetItem* cellItem=ui->tableInfo->item(current->row(),MainWindow::colName); //取当前行第1列的单元格的 item
//    int ID=cellItem->data(Qt::UserRole).toInt();//读取用户自定义数据
//    labStudID->setText(QString::asprintf("学生ID：%d",ID));//学生ID
//}
