...
int main(void)
{
	...
	//char C_str_name[100];
	//scanf("%s",C_str_name);
	char C_str_name[100] = {"DESKTOP-6GJVE4Q"};
	...
	// make  a single item // add item
    std::string changeChanNameName ="ai.float_1" ;//opcItemNames[2];
	...
	itemNames.push_back(opcItemNames[2]);
	itemNames.push_back(opcItemNames[3]);	
	...
	for (i = 0; i < propDesc.size(); i++){
		if (propVals[i] == NULL){
            ...
            printf("propDesc[i].type = %u  ", propDesc[i].type);
			switch (propDesc[i].type)// obtain data type
			{
                ...
                case VT_DATE:printf("%f\n", propVals[i]->value.date); break;
                default:printf("error type: %d \n", propDesc[i].type);break;
			}
		}
	}
