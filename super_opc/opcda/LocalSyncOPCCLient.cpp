...
/*	if (!DetectService("OpcEnum"))
    ...
    */
    ...
    p_opc_server_->getItemNames(items);
	...
	{
        if (ItemNameFilter(items[i]))
		{

            name_item_map_[items[i]] = p_group_->addItem(items[i], true);
            name_item_map_[items[i]]->getSupportedProperties(propDesc);
            name_item_map_[items[i]]->getProperties(propDesc,propVals);

            item_property[items[i]]=make_tuple(propVals[0]->value.iVal,propVals[2]->value.iVal,propVals[4]->value.lVal);
			item_index += 1;
		}
	}
...
bool LocalSyncOPCCLient::DetectService(const char* ServiceName)
{
	// open manager
...
int16_t LocalSyncOPCCLient::Readint16(std::string item_name)
{
    SyncReadItem(item_name, &read_tmp_variant_);
    return read_tmp_variant_.iVal;
}


bool LocalSyncOPCCLient::Writeint16(std::string item_name, int16_t item_value)
{
    static VARIANT var;
    static VARTYPE a = (var.vt = VT_I2);
    var.iVal = item_value;
    SyncWriteItem(item_name, &var);
    return true;
}
