...
// Client Creation
 ...
 /*   while(true)
    {
        for(int type=0x01;type<=0x10;type++)
        {
            int res=Client->SetConnectionType(type);
            if(res==0)
                printf("Client->SetConnectionType ok %d\n",type);
            else
                printf("Client->SetConnectionType fail %d ",res);
           for(int slot=0;slot<=31;slot++)
           {
                int res=Client->ConnectTo(Address,0,slot);
                if(res==0)
                {
                    printf("ConnectTo ok type: %d  slot: %d\n",type,slot);
                    return 0 ;
                }
                else
                    printf("ConnectTo fail %d 0x: %x\n",res,res);
           }
           Sleep(100);
        }
        Sleep(100);
...
// Connection
    ...
    if (CliConnect())
    {
        PerformTests();
        char data[100];
        int res=Client->DBRead(300,0,100,data);
        if (Check(res,"DB READ")) {
              printf(" DB READ OK");
